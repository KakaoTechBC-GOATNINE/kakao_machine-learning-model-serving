...
class ReviewRankingModel:
    ...
    def train_sentiment_model(self, test_size=0.2, random_state=42):
        ...
        positive_keywords = ['좋다', '맛있다', '추천', '최고', '훌륭하다', '기분좋다'] # 긍적적인 리뷰 더 있으면 추가
        negative_keywords = ['나쁘다', '별로', '불친절', '실망', '싫다', '최악'] # 주정적인 리뷰 더 있으면 추가
        ...
        X_train, X_test, y_train, y_test = train_test_split(self.df['processed_review'], self.df['label'], test_size=test_size, random_state=random_state)
    ...
    def rank_reviews(self):
        ...
        # 최종 점수 계산 및 랭킹
        self.df['final_score'] = self.df['sentiment'] * self.df['similarity_score']
    ...
    def save_ranked_reviews(self, output_file_path):
        ...
        rank_df = pd.DataFrame({
            ...
            'Name' : self.df['Name'],
            'processed_review': self.df['processed_review'],  
