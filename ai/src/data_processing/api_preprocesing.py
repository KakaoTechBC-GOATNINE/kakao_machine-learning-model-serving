...
output_csv_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', 'data', 'processed', 'api_preprocessed_reviews.csv')
...
def okt_preprocess(text):
    tokens = okt.pos(text, stem=True)  # 형태소와 품사를 함께 가져옴
    filtered_tokens = []
    for word, pos in tokens:
        if pos in ['Noun', 'Verb', 'Adjective']:  # 명사, 동사, 형용사만 사용
            if word not in stopwords and len(word) > 1:
                if re.match("[가-힣]+", word):  # 한국어가 아닌 것들 제거
